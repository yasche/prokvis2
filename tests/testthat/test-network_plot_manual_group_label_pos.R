...
test_that("Vectors are interlaced", {
  ...
  actual_nums <- str_remove_all(kinase_groups_to_custom_xy(kinase_groups), "custom_group_pos_[a-z]")

  expected_xy <- rep(c("x", "y"), length(kinase_groups))
  actual_xy <- str_remove_all(str_remove_all(kinase_groups_to_custom_xy(kinase_groups), "custom_group_pos_"), "[0-9]")
